...
export default function FileUpload({ user }) {
  ...
  return (
    <div className="p-4 w-[95%] border border-gray-200 rounded flex  items-center">
        <FaFileUpload className="text-blue-500 mr-2 h-6 w-6" /> 
      <input type="file" onChange={handleFileChange} className="mb-2" />
      <button onClick={handleUpload} className="px-2 py-2 bg-blue-500 text-white rounded">
        <FaUpload className="text-white hover:text-green-600 flex gap-2" /> Upload
        Upload
      </button>
    </div>
  );
