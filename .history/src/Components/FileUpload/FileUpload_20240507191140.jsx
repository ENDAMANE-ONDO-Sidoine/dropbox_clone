...
export default function FileUpload({ user }) {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      await GraphQLAPI.graphql(graphqlOperation(createFile, fileDetails.input));
  ...
  return (
    <div className="p-4 w-[93%] border rounded flex justify-center items-center mx-auto border-blue-600 bg-white shadow">
      ...
      <button onClick={handleUpload} disabled={loading} className="px-2 py-2 bg-blue-500 text-white rounded flex gap-2 items-center hover:bg-green-600 transform transition duration-500 ease-in-out hover:scale-110 font-bol">
