...
const FileUploader = () => {
    ...
    const onFileChange = (event) => {
        setFile(event.target.files[0]);  // Updates state with the new file
    };
    ...
    const onFileUpload = async () => {
        if (file) {
            ...
            try {
                // Attempts to upload the file using Amplify Storage
    ...
    return (
        <div className='p-[8%] rounded shadow transform transition duration-500 ease-in-out hover:scale-105 flex gap-4 items-center justify-center h-full bg-slate-300 w-[93%] mx-auto'>
            ...
            <p className={`transition-all duration-500 ${uploadStatus === 'Uploading...' ? 'text-blue-500' : 'text-green-500'}`}>{uploadStatus}</p>
        </div>
    );
};
