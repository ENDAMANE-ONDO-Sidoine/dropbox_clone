...
export default function FileUpload({ user }) {
  ...
  const handleFileChange = (event) => {
    ...
    setUploadSuccess(false); // Réinitialise le message de succès lorsqu'un nouveau fichier est sélectionné
  };
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      setUploadSuccess(true); // Définit le succès du téléchargement sur vrai lorsque le fichier est téléchargé avec succès
    } catch (error) {
  ...
  return (
    <div className="p-4 w-[93%] border  rounded flex  justify-center items-center mx-auto border-blue-600 bg-white shadow">
      ...
      {uploadSuccess && <p style={{ color: 'green', fontWeight: 'bold' }}>Le fichier a été importé avec succès!</p>}
    </div>
  );
}
