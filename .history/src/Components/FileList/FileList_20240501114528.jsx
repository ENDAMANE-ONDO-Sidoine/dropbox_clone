...
import { API, graphqlOperation } from '@aws-amplify/api';  // Adjusted import based on Amplify API
...
export default function FileList() {
  ...
  useEffect(() => {
    const fetchFiles = async () => {
      ...
      try {
        ...
        console.log('File Data:', fileData);  // Debugging output
        ...
        setError('Failed to fetch files. See console for details.');
      } finally {
        setIsLoading(false);
      }
    };
  ...
  return (
    <div className="px-12">
      <h3 className="mb-2 text-2xl font-extrabold text-blue-600 flex gap-2 items-center">
        <FaFolderOpen className='text-yellow-500'/>LIST OF FILES
      </h3>
      {isLoading ? (
        <p>Loading files...</p>
      ) : error ? (
